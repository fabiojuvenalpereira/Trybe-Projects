...
class ProductDetails extends React.Component {
  ...
  async randomImage() {
    ...
    if ('pictures' in product) {
      const numImages = product.pictures.length;
      const randomNum = Math.random() * (numImages - 1) + 1;
      const numToSearch = randomNum.toFixed(0) - 1;
      const image = product.pictures[numToSearch].url;
      this.setState({
        imageUrl: image,
      });
    }
